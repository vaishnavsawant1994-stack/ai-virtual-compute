...
| devices | id, tenant_id, provider_id, host_id, class, persistence, state, version, lifecycle_expires_at |
...
| device_leases | id, tenant_id, session_id, principal_id, client_id, scope, state, issued_at, expires_at, revoked_at |
...
- Every authority-sensitive lookup includes canonical tenant scope before returning a device, session, lease, snapshot, volume, mount, credential reference, event, quota, budget, or usage record.
